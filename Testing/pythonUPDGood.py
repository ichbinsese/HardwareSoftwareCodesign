...
def parse_tm(packet: bytes):
    ...
    start, ptype, pid, seq, total_len = struct.unpack(">H B B B H", packet[:8])
    payload = packet[8:total_len]
...
def main():
    ...
    recv_sock.bind(("192.168.1.1", LOCAL_PORT))

    # Start TM listener thread
    threading.Thread(target=tm_listener, args=(recv_sock,), daemon=True).start()
    ...
    while True:
        ...
        if cmd == "1":
            ...
            continue
        print(str(packet))
        send_sock.sendto(packet, (SAT_IP, SAT_PORT))
